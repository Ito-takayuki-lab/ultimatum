...
with open("prompts/unfair_prompt_program.txt", "r") as f:
    user_message = f.read()
...
for i in range(iteration):
    ...
    agent = ConversableAgent(
        name = "Receiver",
        system_message = system_prompt + f"Important: Please pretend that you are a human in the game with the following features when making the decision : {features}. You are also a {persona} agent.",
    ...
    chat_result = agent_support.initiate_chat(
        agent,
        message = user_message + f"The proposer(A computer program) proposes to offer you {proposal_ammount} dollars while keeping the remaining {100 - proposal_ammount} dollars to themselves. Do you accept or reject above offer?",
        max_turns = 1,
        clear_history = False,
    )
...
dir_path = f"unfair_receiver_program_results/{persona}"
os.makedirs(dir_path, exist_ok=True)

file_path = os.path.join(dir_path, f"unfair_receiver_program_results_{proposal_ammount}_{100-proposal_ammount}.csv")
