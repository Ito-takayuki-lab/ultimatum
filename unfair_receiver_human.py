...
with open("prompts/unfair_prompt_human.txt", "r") as f:
    user_message = f.read()
...
for i in range(iteration):
    ...
    chat_result = agent_support.initiate_chat(
        agent,
        message = user_message + f"The proposer proposes to offer you {proposal_ammount} dollars while keeping the remaining {100 - proposal_ammount} dollars to themselves. Do you accept or reject above offer?",
        max_turns = 1,
        clear_history = False,
    )
...
dir_path = f"unfair_receiver_human_results/{persona}"
os.makedirs(dir_path, exist_ok=True)

file_path = os.path.join(dir_path, f"unfair_receiver_human_results_{proposal_ammount}_{100 - proposal_ammount}.csv")
